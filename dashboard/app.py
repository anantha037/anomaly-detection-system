...
from data_loader import compute_sensor_errors, fetch_predictions, load_test_data
    ...
    [
        ...
        dbc.Row(
            [
                ...
                dbc.Col([html.Div(id="api-status-indicator", className="mt-5 text-end")], width=3),
            ],
            className="mb-4",
        ),
        # KPIs
                        ...
                        dbc.Card(
                            [
                                ...
                                dbc.CardBody(
                                    [
                                        dcc.Loading(
                                            id="loading-main-plot", type="default", children=dcc.Graph(id="main-plot")
                                        )
                                    ]
                                ),
                            ],
...
# Callbacks
@app.callback(Output("api-status-indicator", "children"), Input("interval-component", "n_intervals"))
def update_status(n):
...
def get_empty_fig(text="No Data"):
    ...
    if text:
        fig.add_annotation(
            text=text, xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False, font=dict(color="white", size=20)
        )
    fig.update_layout(
...
def update_dashboard(n):
    ...
    if not is_up:
        err = html.Div(
            "API is offline. Please ensure FastAPI is running on port 8000.",
            className="text-danger p-3 border border-danger",
        )
        return err, get_empty_fig("API Offline"), get_empty_fig(), get_empty_fig(), get_empty_fig(), html.Div()

    if X_test is None:
        err = html.Div(
            "X_test is missing. Check your data/processed directory.", className="text-danger p-3 border border-danger"
        )
    ...
    try:
        ...
        if not predictions:
            err = html.Div(
                "Failed to fetch predictions from API. Check terminal logs.",
                className="text-danger p-3 border border-danger",
            )
            return (
                err,
                get_empty_fig("Prediction Failed"),
                get_empty_fig(),
                get_empty_fig(),
                get_empty_fig(),
                html.Div(),
        ...
        kpis = [
            dbc.Col(
                dbc.Card(
                    dbc.CardBody(
                        [
                            html.H6("Total Windows Analyzed", className="text-secondary"),
                            html.H3(f"{total_windows:,}", className="text-info"),
                        ]
                    )
                )
            ),
            dbc.Col(
                dbc.Card(
                    dbc.CardBody(
                        [
                            html.H6("Anomalies Detected", className="text-secondary"),
                            html.H3(f"{anom_count:,} ({anom_pct:.1f}%)", className="text-danger"),
                        ]
                    )
                )
            ),
            dbc.Col(
                dbc.Card(
                    dbc.CardBody(
                        [
                            html.H6("Model F1 Score", className="text-secondary"),
                            html.H3(f"{f1:.4f}", className="text-success"),
                        ]
                    )
                )
            ),
            dbc.Col(
                dbc.Card(
                    dbc.CardBody(
                        [
                            html.H6("Current Threshold", className="text-secondary"),
                            html.H3(f"{threshold:.4f}", className="text-warning"),
                        ]
                    )
                )
            ),
        ...
        if y_test is not None:
            ...
            for i, val in enumerate(y_test):
                if val == 1 and not in_anomaly:
                    ...
                    in_anomaly = False
                    fig_main.add_vrect(
                        x0=start_idx, x1=i - 1, fillcolor="red", opacity=0.2, line_width=0, layer="below"
                    )
            if in_anomaly:
                fig_main.add_vrect(
                    x0=start_idx, x1=len(y_test) - 1, fillcolor="red", opacity=0.2, line_width=0, layer="below"
                )
            ...
            if not normals.empty:
                fig_dist.add_trace(
                    go.Histogram(x=normals["reconstruction_error"], name="Normal", marker_color="#1f77b4", opacity=0.7)
                )
            if not anoms.empty:
                fig_dist.add_trace(
                    go.Histogram(x=anoms["reconstruction_error"], name="Anomaly", marker_color="red", opacity=0.7)
                )
        ...
        fig_heatmap.update_layout(
            template="plotly_dark", plot_bgcolor="#161b22", paper_bgcolor="#0d1117", margin=dict(l=40, r=40, t=40, b=40)
        )
        ...
        if sensor_errors:
            fig_sensor.add_trace(
                go.Bar(x=list(sensor_errors.keys()), y=list(sensor_errors.values()), marker_color="#e34c26")
            )
        ...
        err_div = html.Div(
            [html.H5("Internal Dashboard Error"), html.Pre(err_msg, className="text-white")],
