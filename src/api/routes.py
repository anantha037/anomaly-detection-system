...
from src.api.models import PredictRequest, PredictResponse, WindowResult, ThresholdResponse, MetricsResponse, HealthResponse
...
def health_check(state: dict = Depends(get_app_state)):
    return HealthResponse(
        ...
        threshold_loaded=state.get("threshold") is not None
    )
...
def predict(request: PredictRequest, state: dict = Depends(get_app_state)):
    ...
    return PredictResponse(
        results=results,
        anomaly_count=summary["total_alerts"],
        total_windows=len(X),
